...
const Info: React.FC<Accountpropes> = ({ user, isYourAccount }) => {
  ...
  const updateprofilehandler = async () => {
    await updateUser(name, phone_number, bio);
    setOpen(false);
  };
        ...
        <div className="h-32 bg-blue-500 relative">
          <div className="absolute -bottom-16 left-6">
            <div className="relative group">
              <div className="w-32 h-32 rounded-full border-4 overflow-hidden shadow-xl bg-background">
                <img
                  src={user.profile_pic ? user.profile_pic : "/user.webp"}
                  alt=""
                  className="w-full h-full object-cover"
                />
              </div>
              {/* Edit for your account pic */}
              {isYourAccount && (
                <>
                  <Button
                    variant={"secondary"}
