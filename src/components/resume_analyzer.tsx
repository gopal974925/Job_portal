...
const ResumeAnalyzer = () => {
  ...
  const analyzeResume = async () => {
    ...
    try {
      ...
      const { data } = await axios.post(
        `${utils_service}/api/career/resume`,
        {
          pdfBase64: base64,
        }
      );
  ...
  const scoreBreakdown = response?.scoreBreakdown || response?.scoreBreakedown || {};
  const strengths = response?.strengths || response?.strenghs || [];
                          ...
                          <div key={index} className="p-4 rounded-lg border">
                            <div className="flex items-start justify-between gap-3 mb-2">
                              <h4 className="font-semibold text-sm">
                                {suggestion.category || suggestion.catagory}
                              </h4>
                              <span
